...
export default function AudioEdit({ audio, speakers, categories, years }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT', // Laravel method spoofing for multipart forms
        title: audio.title,
        ...
        status: audio.status,
        speaker_id: audio.speaker_id.toString(),
        category_id: audio.category_id.toString(),
        year_id: audio.year_id.toString(),
        tags: audio.tags,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Since we are uploading files, we must use POST with _method = PUT spoofing
        post(`/admin/audio/${audio.id}`);
    };

    const getAudioPreviewUrl = () => {
        if (!audio.uri) return undefined;
        if (audio.uri.startsWith('http')) return audio.uri;
        return `/storage/media/${audio.uri}`;
    };

    const getThumbnailPreviewUrl = () => {
        if (!audio.thumbnail_uri) return undefined;
        if (audio.thumbnail_uri.startsWith('http')) return audio.thumbnail_uri;
        return `/storage/thumbnails/${audio.thumbnail_uri}`;
    };
    ...
    return (
        ...
        >
            <Head title={`Edit Audio — ${audio.title}`} />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <FileUpload
                                    ...
                                    hint="Leave empty to keep existing file. Max 100MB"
                                    preview={getAudioPreviewUrl()}
                                />
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <FileUpload
                                    ...
                                    hint="Leave empty to keep existing image. Max 5MB"
                                    preview={getThumbnailPreviewUrl()}
                                />
                            </div>
                            <div className="flex flex-col gap-4">
                                <Input
